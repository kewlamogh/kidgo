...
AWS_ACCESS_KEY_ID = 'AKIAVEFBPAWBH4BVIPUM'
AWS_SECRET_ACCESS_KEY = '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'
...
def inbox():    
    ...
    'https://xujhofn50c.execute-api.ap-south-1.amazonaws.com/production/inbox?username=' + request.args.get('user').lower() + "&classroom=" + request.args.get("class").lower())

    print('https://xujhofn50c.execute-api.ap-south-1.amazonaws.com/production/inbox?username=' +
...
def sendmessage():
    ...
    response = requests.get(
        'https://xujhofn50c.execute-api.ap-south-1.amazonaws.com/production/send?from=' + from_param + "&to=" + to_param + "&s3url=" + quote(s3_url) + "&classroom=" + classid.lower())
...
def outbox():    
    response = requests.get(
        'https://xujhofn50c.execute-api.ap-south-1.amazonaws.com/production/outbox?username=' + request.args.get('user').lower() + "&classroom=" + request.args.get("class").lower())
